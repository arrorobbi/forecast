...
const detail = async (req, res, next) => {
  try {
    const { id } = req.params;
    const data = await forecast_services.getbyId(id);
    return res.status(200).json({
      status: 200,
      message: "Request Success",
      payload: data,
...
const update = async (req, res, next) => {
  try {
    //passing getAll service
    const { id } = req.params;
    const { actual_forecast } = req.body;
    console.log(actual_forecast);
    const get_data = await forecast_services.getbyId(id);
    const actual_upper =
      ((actual_forecast - get_data.dataValues.RTS) / get_data.dataValues.RTS) *
      100;
    const payload = {
      actual_forecast: actual_forecast,
      actual_upper: actual_upper.toFixed(2),
    };

    const data = await forecast_services.update(id, payload);
